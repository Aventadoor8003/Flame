        ...
        post("/submit", (request, response) -> {
            ...
            for (int i = 0; i < threads.length; i++) {
                try {
                    ...
                    return "Interrupted while uploading JAR file to worker " + getWorkers().elementAt(i);
                }
            }
            ...
            FlameContextImpl context = new FlameContextImpl(kvs);
            try {
            ...
            } catch (TableRenameException e) {
                response.status(500, "Internal server error");
                logger.error("Failed to rename table", e);
                return "Failed to rename table";
            } catch (Exception e) {
                response.status(500, "Internal server error");
                logger.error("Failed to invoke run method", e);
                return "Failed to invoke run method";
            }
