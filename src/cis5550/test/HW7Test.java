...
public class HW7Test extends GenericTest {
  ...
  public static void main(String args[]) throws Exception {
    ...
    if ((args.length == 0) || args[0].equals("all") || args[0].equals("auto")) {
      tests.add("count");
      tests.add("save"); //add test: wrong tableNameArg; save in pairRDD
      tests.add("take"); //check test: when the parameter is exactly the number of elements in the table
      tests.add("fromtable"); //better understand the test and try to add more tests
      tests.add("map1"); // add test: use another lambda expression to test Rdd.flatMapToPair
      tests.add("map2");
      tests.add("map3");
      tests.add("distinct");
      tests.add("join");
